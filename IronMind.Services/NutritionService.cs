...
public class NutritionService(INutritionRepository nutrition, IUserRepository users, IHttpClientFactory httpClientFactory) : INutritionService
{
    public async Task<MealLogDto> LogMealAsync(int userId, LogMealRequest request)
    {
        ...
        await nutrition.AddMealLogAsync(log);
        await nutrition.SaveChangesAsync();
        return ToDto(log);
    }
    ...
    {
        var logs = await nutrition.GetMealLogsForDateAsync(userId, date);
        return logs.Select(ToDto);
    }

    public async Task<MealLogDto> UpdateMealLogAsync(int userId, int logId, UpdateMealRequest request)
    {
        var log = await nutrition.GetMealLogAsync(userId, logId)
            ?? throw new KeyNotFoundException("Meal log not found.");

        log.FoodName = request.FoodName;
        log.Calories = request.Calories;
        log.ProteinG = request.ProteinG;
        log.CarbsG = request.CarbsG;
        log.FatG = request.FatG;
        log.OpenFoodFactsId = request.OpenFoodFactsId;
