...
public record UpdateCardioRequest(string ActivityType, int DurationMinutes,
    float? DistanceKm = null, string? Notes = null);

public record LogStrengthRequest(string ExerciseName, IEnumerable<SetEntry> Sets, string? Notes = null);

public record UpdateStrengthRequest(string ExerciseName, IEnumerable<SetEntry> Sets, string? Notes = null);
